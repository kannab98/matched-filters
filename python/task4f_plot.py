...
df = pd.read_excel('task4f.xlsx', engine='openpyxl')
print(df)
...
N = 2
# Шаг по длительности
t =  np.linspace(400, 1000, 70)
tau = np.array([t.tolist() for i in range(N)])
...
index = np.where((out > 1000) & (out < 2000) )
outm = outm[index]
out = out[index]
tau = tau[index]

index = np.where((outm > 1000) & (outm < 2000) )
outm = outm[index]
out = out[index]
tau = tau[index]
...
ax.set_xlabel("Девиация частоты $\\Delta f$, Гц")
ax.set_ylabel("ОСШ")
legend = plt.legend(handles=[l1[0], l2[0]], loc=2)
plt.gca().add_artist(legend)

f = np.polyfit(tau, out, 0)
k = f[0]
f = np.poly1d(f)
fm = np.polyfit(tau, outm, 0)
km = fm[0]
fm = np.poly1d(fm)


l3 = ax.plot(tau, f(tau), 'r', label="$k=%.1f$" % k)
l4 = ax.plot(tau, fm(tau), 'b', label="$k=%.1f$" % km)


# # legend = plt.legend(handles=[l3[0], l4[0]], loc=4)
# # plt.gca().add_artist(legend)
fig.savefig("../imgs/task4/t4ff1.pdf", bbox_inches="tight")
...
# plt.gca().add_artist(legend)
# fig.savefig("../imgs/task4/t4ff2.pdf")
# # ax.legend()
