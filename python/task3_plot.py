...
df = pd.read_excel('task3.xlsx', engine='openpyxl', sheet_name=[0,1,2])

xlabel = [r"$\Delta f_{\text{дев}}$, Гц", r"$\tau_{\text{вх}}$, мс", r"$\tau_{\text{вх}}$, мс"]
ylabel = [r"$\tau_{\text{вых}}$, мс", r"$A_{\text{вых}}$", r"$t^*_{\text{вых}}$, мс"]
label = []

for i in range(3):
    fig, ax = plt.subplots()
    ax.plot(df[i].iloc[:,0], df[i].iloc[:, 1])
    ax.set_xlabel(xlabel[i])
    ax.set_ylabel(ylabel[i])
    fig.savefig("../imgs/task3/t3f%s" % i, bbox_inches="tight")
